...
from recommendation_service.src.main import app
...
from recommendation_service.src.repositories.beanie_repository import (
    BeanieRepository, CreateSchemaType
)
from ..src.models.base import CreateMixin
from ..src.repositories.completion.video_completion import video_completion_repository
...
def make_recommendations_client() -> TestClient:
    """
    Данная фикструа создает клиента для FastAPI Auth сервиса, при этом идет инициализация приложения FastAPI по всему циклу,
    включая start/down events (lifespan - метод), тем самым нам не требуется запускать экземпляр приложения
    при отладке тестов Fast API.
    """
    with TestClient(
            app=app, base_url=settings.local.host + ":" + settings.local.port + '/api/v1/recommendations'
    ) as auth_api_test_client:
        auth_api_test_client.headers.setdefault('X-Request-Id', f'X_REQUEST_ID_{uuid4().hex[:6]}')
...
def _generate_es_actions(index_name: str, data: Iterable[dict[str, Any]]):
    ...
    return [{'_index': index_name, '_id': doc['id'], '_source': doc} for doc in data]


async def _setup_index(es_client: AsyncElasticsearch, index: IndexSchema, data: list[dict[str, Any]]):
...
def _generate_mongo_entries(create_class_name: CreateMixin, data: Iterable[dict[str, Any]]):
    """Оборачиваем данные для вставки в Mongo"""
    return [create_class_name(**entry) for entry in data]


async def _setup_mongo_table(reposiotory: BeanieRepository, create_class_name: CreateMixin, data: list[dict[str, Any]]):
    # заполняем таблицу
    entries = _generate_mongo_entries(create_class_name, data)
    for entry in entries:
        await reposiotory.create(entry)


async def _teardown_mongo_table(reposiotory: BeanieRepository, data: list[dict[str, Any]]):
    for entry in data:
        await reposiotory.delete(
            {
                "id": {
                    "comparison": "=",
                    "value": entry["id"],
...
async def mongo_setup(es_client: AsyncElasticsearch):
    await _setup_mongo_table(video_completion_repository, VideoCompletionCreate, history.data)
