...
@pytest.mark.anyio
async def test_repeatable_calls_same_params_return_consistent_type(recommendation_service):
    """
    Два одинаковых вызова подряд не падают.
    Допускаем, что сервис вернёт None при пустой истории/недоступных внешках.
    """
    _ensure_completion_stub(recommendation_service)
    uid = "043ce182-bef0-467e-9362-13d514e57837"
    r1 = await recommendation_service.get_recommendations(user_id=uid, search_values={})
    r2 = await recommendation_service.get_recommendations(user_id=uid, search_values={})
...
async def test_service_resilience_does_not_raise(recommendation_service):
    """
    Мягкая устойчивость: вызов не валит тесты даже при сбоях внешних систем.
