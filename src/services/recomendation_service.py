...
from recommendation_service.src.services.movies.films import FilmService, get_film_service


class RecommendationService:

    def __init__(self, completion_service: VideoCompletionService, films_service: FilmService):
        self.completion_service = completion_service
        self.films_service = films_service

    async def get_recommendations(self, user_id: str, search_values: dict[str:dict]):
        films_history = await self.get_history(user_id)
        if len(films_history):
            films_ids_to_percentage = {history.film_id: films_history.watched_percentage for history in films_history}
            films = await self.films_service.search_by_ids(films_ids_to_percentage.keys())
            for film in films:
    ...
    def add_elements_to_search_fields(self, search_values, film_field, watched_percentage):
